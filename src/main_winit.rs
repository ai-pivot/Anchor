...
impl TileNode {
    fn clone_shallow(&self) -> Self {
        match self {
            TileNode::Window { surface } => TileNode::Window {
    ...
    fn layout_rec(
        ...
        match self {
            ...
            TileNode::Split {
                ...
                if children.len() == 1 {
                    return children[0].layout_rec(area, out);
                }
                let gap = 4;
                let n = children.len() as i32;
                let total = match dir {
                    SplitDir::Horizontal => area.size.w,
                    SplitDir::Vertical => area.size.h,
                };
                ...
                for (i, child) in children.iter().enumerate() {
                    ...
                    let ca = match dir {
                        SplitDir::Horizontal => Rectangle {
                            loc: Point::new(area.loc.x + offset, area.loc.y),
                            size: Size::new(sz, area.size.h),
                        },
                        SplitDir::Vertical => Rectangle {
                            loc: Point::new(area.loc.x, area.loc.y + offset),
                            size: Size::new(area.size.w, sz),
                        },
                    };
                    child.layout_rec(ca, out);
                    offset += sz + gap;
                }
            }
        }
    ...
    fn insert_next_to(&mut self, t: &WlSurface, node: TileNode, dir: SplitDir) -> bool {
        match self {
            TileNode::Window { surface } if surface == t => {
                ...
                if let TileNode::Split { children, .. } = self {
                    children.push(old);
                    children.push(node);
                }
                true
            }
            TileNode::Split { children, .. } => children
                .iter_mut()
                .any(|c| c.insert_next_to(t, node.clone_shallow(), dir)),
            _ => false,
        }
    }
    ...
    fn all_surfaces(&self) -> Vec<WlSurface> {
        let mut v = vec![];
        self.collect(&mut v);
        v
    }
    fn collect(&self, o: &mut Vec<WlSurface>) {
        match self {
            TileNode::Window { surface } => o.push(surface.clone()),
            TileNode::Split { children, .. } => children.iter().for_each(|c| c.collect(o)),
        }
    }
    fn next_after(&self, t: &WlSurface) -> Option<WlSurface> {
        let a = self.all_surfaces();
        a.iter()
            .position(|s| s == t)
            .map(|i| a[(i + 1) % a.len()].clone())
            .or_else(|| a.first().cloned())
    }
    fn prev_before(&self, t: &WlSurface) -> Option<WlSurface> {
        let a = self.all_surfaces();
        a.iter()
            .position(|s| s == t)
            .map(|i| a[if i == 0 { a.len() - 1 } else { i - 1 }].clone())
            .or_else(|| a.last().cloned())
    }
}
...
impl XdgShellHandler for App {
    ...
    fn new_toplevel(&mut self, s: ToplevelSurface) {
        ...
        if let Some(ref f) = self.focus {
            if !self
                .root
                .insert_next_to(f, n.clone_shallow(), self.next_dir)
            {
                self.append_to_root(n);
            }
        } else {
            self.append_to_root(n);
        }
...
impl CompositorHandler for App {
    ...
    fn client_compositor_state<'a>(&self, c: &'a Client) -> &'a CompositorClientState {
        &c.get_data::<ClientState>().unwrap().comp
    }
...
impl App {
    fn new(dh: &DisplayHandle) -> Self {
        App {
            ...
            seat: SeatState::new().new_wl_seat(dh, "seat0"),
            ...
            focus: None,
            next_dir: SplitDir::Horizontal,
            osize: Size::new(1280, 720),
            tops: vec![],
            run: true,
            dh: dh.clone(),
        }
    ...
    fn append_to_root(&mut self, node: TileNode) {
        match &mut self.root {
            ...
            _ => {
                ...
                if let TileNode::Split { children, .. } = &mut self.root {
                    children.push(old);
                    children.push(node);
                }
            }
    ...
    fn set_focus(&mut self, s: &WlSurface) {
        if let Some(ref old) = self.focus {
            if old != s {
                if let Some(t) = self.find_top(old) {
    ...
    fn find_top(&self, s: &WlSurface) -> Option<&ToplevelSurface> {
        self.tops.iter().find(|t| t.wl_surface() == s)
    }
    fn remove_win(&mut self, s: &WlSurface) {
        if self.focus.as_ref() == Some(s) {
            self.focus = self.root.next_after(s);
        }
        if self.root.remove(s) && self.root.all_surfaces().is_empty() {
    ...
    fn fnxt(&mut self) {
        if let Some(ref c) = self.focus {
            if let Some(n) = self.root.next_after(c) {
                self.set_focus(&n);
            }
        }
    }
    fn fprv(&mut self) {
        if let Some(ref c) = self.focus {
            if let Some(p) = self.root.prev_before(c) {
                self.set_focus(&p);
            }
        }
    }
...
fn handle_sym(app: &mut App, sym: &Keysym, shift: bool) {
    ...
    if *sym == k(keysyms::KEY_Return) && !shift {
        spawn_term();
    } else if *sym == k(0x0071) && shift {
        let s = app.focus.clone();
        if let Some(s) = s {
            app.remove_win(&s);
        }
    } else if (*sym == k(keysyms::KEY_Left) || *sym == k(keysyms::KEY_Up)) && !shift {
        app.fprv();
    } else if (*sym == k(keysyms::KEY_Right) || *sym == k(keysyms::KEY_Down)) && !shift {
        app.fnxt();
    } else if *sym == k(0x0076) && shift {
        app.next_dir = SplitDir::Vertical;
        info!("📐 ↓");
    } else if *sym == k(0x0062) && shift {
        app.next_dir = SplitDir::Horizontal;
        info!("📐 →");
    } else if *sym == k(keysyms::KEY_space) && shift {
        app.next_dir = match app.next_dir {

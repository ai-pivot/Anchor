...
pub fn render_notifications(
    ...
    for (text, created, duration) in notifications.iter() {
        ...
        f.clear(
            opaque(0.06 * bg_alpha, 0.06 * bg_alpha, 0.10 * bg_alpha),
        ...
        f.clear(
            ...
            &[rect(nx, ny, 3, content_h)],
        )
        .ok();
        // 发光
        f.clear(
            opaque(
                accent.0 * accent_br * 0.3,
                accent.1 * accent_br * 0.3,
                accent.2 * accent_br * 0.3,
            ),
            &[rect(nx + 3, ny, 3, content_h)],
        )
        .ok();
